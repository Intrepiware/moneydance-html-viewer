...
- Start a 60-second shutdown deadline on the first configured-book closing notification, including any wait for a manual attempt before capture. Preserve each operation's capture-start deadline and use the earlier applicable deadline. Duplicate closing notifications within the active cycle preserve its candidate and deadlines. Opening or confirmed cancellation ends the cycle; verify cancellation detection in the actual runtime before claiming it works. T021 requires three publishing close/reopen cycles with distinct verified edits, not merely three installation restarts.
